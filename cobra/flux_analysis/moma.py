...
def moma(wt_model, mutant_model, objective_sense='maximize', solver='gurobi',
    ...
    if parsimonious:
        ...
        if minimum_flux is None:
            ...
            parsimonious_flux_ceiling = ceil(minimum_flux/tolerance_optimality)*tolerance_optimality
            wt_model = wt_model.copy()
            mutant_model = mutant_model.copy()
            map(convert_to_irreversible, [wt_model, mutant_model])
